...
def main():
	...
	for iter in tqdm(range(N_TIME_STEPS)):
		...
		for i in range(n_particles):
			for j_in_list, j in enumerate(neighbour_ids[i]):
				densities[i] += NORMALIZATION_DENSITY * (
					SMOOTHING_LENGHT**2 - distances[i][j_in_list]**2
				)**3
		...
		# Drop the elemen itself
		neighbour_ids = [ np.delete(x, 0) for x in neighbour_ids]
		...
		for i in range(n_particles):
			for j_in_list, j in enumerate(neighbour_ids[i]):
				...
				forces[i] += NORMALIZATION_VISCOUS_FORCE * (
					...
					(
						SMOOTHING_LENGHT - distances[i][j_in_list]
					)
				)
		...
		velocities[out_of_bottom_boundary, 0] *= DAMPING_COEFFICIENT
		positions [out_of_bottom_boundary, 0] = DOMAIN_Y_LIM[0]

		velocities[out_of_bottom_boundary, 0] *= DAMPING_COEFFICIENT
		positions [out_of_bottom_boundary, 0] = DOMAIN_Y_LIM[1]
